...
use std_shims::{collections::HashMap, string::String, sync::LazyLock, vec::Vec};
...
static LANGUAGES: LazyLock<HashMap<Language, WordList>> = LazyLock::new(|| {
    HashMap::from([
        (
            Language::Czech,
            WordList::new(include!("./words/cs.rs"), true, false, " ", false),
        ),
        (
            Language::French,
            WordList::new(include!("./words/fr.rs"), true, true, " ", true),
        ),
        (
            Language::Korean,
            WordList::new(include!("./words/ko.rs"), false, false, " ", true),
        ),
        (
            Language::English,
            WordList::new(include!("./words/en.rs"), true, false, " ", false),
        ),
        (
            Language::Italian,
            WordList::new(include!("./words/it.rs"), true, false, " ", false),
        ),
        (
            Language::Spanish,
            WordList::new(include!("./words/es.rs"), true, true, " ", true),
        ),
        (
            Language::Japanese,
            WordList::new(include!("./words/ja.rs"), false, false, "\u{3000}", true),
        ),
        (
            Language::Portuguese,
            WordList::new(include!("./words/pt.rs"), true, false, " ", false),
        ),
        (
            Language::ChineseSimplified,
            WordList::new(
                include!("./words/zh_simplified.rs"),
                false,
                false,
                " ",
                false,
        ...
        (
            Language::ChineseTraditional,
            WordList::new(
                include!("./words/zh_traditional.rs"),
                false,
                false,
                " ",
                false,
    ...
    fn to_poly(&self) -> Zeroizing<[u16; POLYSEED_LENGTH]> {
        ...
        for i in 0..DATA_WORDS {
            ...
            while word_bits < SECRET_BITS_PER_WORD {
                ...
                word_val <<= chunk_bits;
                word_val |= (u16::from(self.entropy[entropy_idx]) >> secret_bits)
                    & ((1u16 << chunk_bits) - 1);
            }
            ...
            let Some(coeff) = (if lang_word_list.has_accent {
                ...
                check_if_matches(
                    ...
                    lang_word_list
                        .words
                        .iter()
                        .map(|lang_word| strip_accents(lang_word)),
                    &strip_accents(word),
                )
            } else {
