...
fn test_polyseed() {
    ...
    for vector in vectors {
        ...
        let trim_seed = |seed: &str| {
            let seed_to_trim = if vector.has_accent {
                seed_without_accents(seed)
            } else {
                seed.to_string()
            };
            seed_to_trim
        ...
        let seed = Polyseed::from_string(
            vector.language,
            Zeroizing::new(vector.seed.clone()),
            Coin::Monero,
            0,
        )
        .unwrap();
        ...
        let whitespaced_seed = Polyseed::from_string(
            vector.language,
            Zeroizing::new(add_whitespace),
            Coin::Monero,
            0,
        )
        .unwrap();
        ...
        if vector.has_prefix {
            let trimmed_seed =
                Polyseed::from_string(vector.language, Zeroizing::new(trim), Coin::Monero, 0)
                    .unwrap();
            assert_eq!(seed, trimmed_seed);
        }
        // Check versions without accents work
        if vector.has_accent {
            let seed_without_accents = Polyseed::from_string(
                vector.language,
        ...
        assert_eq!(
            seed.to_string(Coin::Monero),
            from_entropy.to_string(Coin::Monero)
        );
        ...
        // Check against ourselves
        {
            ...
            assert_eq!(
                seed,
                Polyseed::from(vector.language, 0, seed.birthday(), seed.entropy().clone(),)
                    .unwrap()
            );
        }
    }
...
fn test_key() {
    let seed: String = "comic blanket chair inject end snow rural improve cereal \
     better initial replace ribbon brother gather unaware"
        .into();
    let res =
        Polyseed::from_string(Language::English, Zeroizing::new(seed), Coin::Monero, 0).unwrap();
...
fn test_is_encrypted() {
    ...
    assert!(
        !seed.is_encrypted(),
        "A newly created seed should not be encrypted"
    );
}
...
fn test_from_string_auto_english() {
    let seed_str = "raven tail swear infant grief assist regular lamp \
      duck valid someone little harsh puppy airport language";
    let (seed, lang) =
        Polyseed::from_string_auto(Zeroizing::new(seed_str.into()), Coin::Monero, 0).unwrap();
    assert_eq!(lang, Language::English);
    let expected = Polyseed::from_string(
        Language::English,
        Zeroizing::new(seed_str.into()),
        Coin::Monero,
        0,
...
fn test_from_string_auto_japanese() {
    let seed_str =
        "うちあわせ　ちつじょ　つごう　しはい　けんこう　とおる　てみやげ　はんとし　たんとう \
      といれ　おさない　おさえる　むかう　ぬぐう　なふだ　せまる";
    let (seed, lang) =
        Polyseed::from_string_auto(Zeroizing::new(seed_str.into()), Coin::Monero, 0).unwrap();
...
fn test_from_string_auto_invalid() {
    let garbage =
        Zeroizing::new("xxx xxx xxx xxx xxx xxx xxx xxx xxx xxx xxx xxx xxx xxx xxx xxx".into());
...
fn test_enable_features() {
    ...
    let seed = Polyseed::from_internal(
        ...
        0,
        entropy.clone(),
        FEATURE_FOO | FEATURE_BAR | FEATURE_QUX,
    )
    .unwrap();
    ...
    let seed_with_feat =
        Polyseed::from(Language::English, FEATURE_FOO, 1635768000, entropy.clone()).unwrap();
    ...
    let decoded =
        Polyseed::from_string(Language::English, phrase, Coin::Monero, FEATURE_FOO).unwrap();
...
fn test_nfc_output_japanese() {
    ...
    assert_eq!(
        ideographic_spaces, 15,
        "Japanese output should have 15 ideographic space separators"
    );
    ...
    assert_eq!(
        ascii_spaces, 0,
        "Japanese output should not contain ASCII spaces"
    );
}
...
fn test_birthday_edge_cases() {
    ...
    assert_eq!(
        birthday_decode(DATE_MASK),
        POLYSEED_EPOCH + u64::from(DATE_MASK) * TIME_STEP
    );
}
...
fn test_encrypted_mnemonic_roundtrip() {
    ...
    assert!(
        ...
        "Decoded seed should still be encrypted"
    );
    assert_eq!(
        *decoded.entropy(),
        *encrypted.entropy(),
        "Encrypted entropy preserved through mnemonic"
    );
