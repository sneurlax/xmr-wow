...
fn bug_accent_strip_uses_ascii_filter() {
    ...
    prision hora regalo lengua existir lapiz lote sonoro"
        ...
        .collect();
    let res_nfc =
        Polyseed::from_string(Language::Spanish, Zeroizing::new(nfc_seed), Coin::Monero, 0);
...
fn bug_poly_arrays_not_zeroized() {
    ...
    let _seed = Polyseed::from_string(
        Language::English,
        Zeroizing::new(seed.into()),
        Coin::Monero,
        0,
...
fn gap_no_multi_coin_support() {
    let seed = "raven tail swear infant grief assist regular lamp \
    duck valid someone little harsh puppy airport language";
    let parsed = Polyseed::from_string(
        Language::English,
        Zeroizing::new(seed.into()),
        Coin::Monero,
        0,
...
fn gap_no_crypt() {
    let seed = "raven tail swear infant grief assist regular lamp \
    duck valid someone little harsh puppy airport language";
    let mut parsed = Polyseed::from_string(
        Language::English,
        Zeroizing::new(seed.into()),
        Coin::Monero,
        0,
...
fn gap_no_auto_detect() {
    ...
    duck valid someone little harsh puppy airport language";
    let (parsed, lang) =
        Polyseed::from_string_auto(Zeroizing::new(seed.into()), Coin::Monero, 0).unwrap();
