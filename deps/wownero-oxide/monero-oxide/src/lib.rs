...
/// Monero serialization primitives plus the legacy `CompressedPoint` re-export.
pub mod io {
  pub use monero_io::*;
  pub use monero_ed25519::CompressedPoint;
}
pub use monero_ed25519 as ed25519;
pub use monero_primitives as primitives;

/// Backward-compatible generator exports retained for downstream users still on the
/// pre-`ed25519` API surface.
pub mod generators {
  use super::ed25519::CompressedPoint;
  use std_shims::sync::LazyLock;

  /// The Monero `H` generator as a lazily decompressed Edwards point.
  pub static H: LazyLock<curve25519_dalek::EdwardsPoint> = LazyLock::new(|| {
    curve25519_dalek::edwards::CompressedEdwardsY(CompressedPoint::H.to_bytes())
      .decompress()
      .expect("couldn't decompress Monero H generator")
  });
}
