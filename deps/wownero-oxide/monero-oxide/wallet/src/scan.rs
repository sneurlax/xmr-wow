      ...
      */
      ...
      for key in tx_keys.iter().map(Some).chain(core::iter::once(additional)).flatten().copied() {
        ...
        // Regular transaction
          ...
          //
          // For WOW RCT type 8 (WowneroClsagBulletproofPlus), commitments are stored
          // as C/8 (scaled by INV_EIGHT). We need to multiply by 8 to recover the
          // full commitment before comparing.
          let rebuilt = commitment.commit();
          let stored_commitment = proofs.base.commitments.get(o);
          let commitment_matches = stored_commitment.map_or(false, |stored| {
