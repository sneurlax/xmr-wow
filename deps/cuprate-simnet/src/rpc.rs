//! monerod-compatible HTTP RPC server backed by a `SimnetNode`.
//!
...
use crate::{error::SimnetError, node::SimnetNode};

// --- shared state -------------------------------------------------------------

type SharedNode = Arc<Mutex<SimnetNode>>;
...
async fn get_blocks_bin(
    ...
    for h in start_height..end {
        ...
        let tx_hashes: Vec<[u8; 32]> = monero_oxide::block::Block::read(
            ...
            for _ in 0..arr_len {
                ...
                for _ in 0..obj_fields {
                    ...
                    match ftype {
                        ...
                        EPEE_UINT8 => {
                            if pos >= data.len() { return Err("u8 oob".into()); }
                            pos += 1;
                        }
                        _ => {
...
fn skip_epee_value(data: &[u8], mut pos: usize, type_byte: u8) -> Result<usize, String> {
    ...
    match base_type {
        EPEE_INT64 | EPEE_UINT64 | EPEE_DOUBLE => { pos += 8; }
        EPEE_UINT32 => { pos += 4; }
        EPEE_INT32 => { pos += 4; }
        EPEE_UINT16 | EPEE_INT16 => { pos += 2; }
        EPEE_UINT8 | EPEE_INT8 => { pos += 1; }
        EPEE_BOOL => { pos += 1; }
        EPEE_STRING => {
...
fn build_get_output_distribution_response(
    start_height: u64,
    distribution: &[u64],
) -> Vec<u8> {
    let mut body = Vec::new();
...
pub async fn start_rpc_server(
    ...
    let app = Router::new()
        ...
        .route("/get_output_distribution.bin", post(get_output_distribution_bin))
