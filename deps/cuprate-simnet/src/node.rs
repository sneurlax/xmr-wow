...
impl SimnetNode {
    pub async fn start() -> Result<Self, SimnetError> {
        Self::start_with_config(SimnetConfig::default()).await
    }

    pub async fn start_with_config(config: SimnetConfig) -> Result<Self, SimnetError> {
        let tmp = TempDir::new()?;
    ...
    pub async fn scannable_block_at(&mut self, height: usize) -> Result<ScannableBlock, SimnetError> {
        ...
        let transactions = if tx_hashes.is_empty() {
            ...
            for t in tx_records {
                let tx = Transaction::<Pruned>::read(&mut t.tx_blob.as_slice())
    ...
    /// the 60-block lock (the caller must check maturity if needed).
    ...
    /// Create a `SimnetDecoyRpc` that provides the wallet decoy-selection traits for this node.
    ///
    /// The `SimnetDecoyRpc` wraps a clone of the read handle so it does not
    /// require mutable access, matching the `&self` requirement of the wallet
    /// interface traits.
    pub fn decoy_rpc(&self) -> SimnetDecoyRpc {
        SimnetDecoyRpc {
...
//
// Simnet-specific simplifications
// --------------------------------
...
// * All coinbase outputs carry the standard 60-block additional timelock.
//   We rely on the `unlocked` field we already return from `rct_outputs_at_indexes`.

/// Implements the wallet decoy-selection traits directly against a cuprate
/// blockchain read handle.
///
/// Obtain one via [`SimnetNode::decoy_rpc`].
#[derive(Clone)]
pub struct SimnetDecoyRpc {
    read_handle: BlockchainReadHandle,
    /// Snapshot of rct_counts at the time of creation, used to distinguish
    /// coinbase outputs (60-block lock) from non-coinbase outputs (no extra lock).
...
impl monero_wallet::interface::ProvidesBlockchainMeta for SimnetDecoyRpc {
...
impl monero_wallet::interface::ProvidesUnvalidatedDecoys for SimnetDecoyRpc {
    ...
    fn ringct_output_distribution(
        ...
        async move {
            ...
            // rct_counts[h] = total RCT outputs after block h.
    ...
    ) -> impl Send + std::future::Future<Output = Result<Vec<Option<[monero_oxide::ed25519::Point; 2]>>, TransactionsError>>
    {
        ...
        async move {
            ...
            // rct_counts is cumulative: block h contains outputs in range
            // rct_counts[h-1]..rct_counts[h].  The coinbase tx is the first tx,
            // and in the simnet it produces exactly 1 output at index rct_counts[h-1].
            ...
            indexes
                ...
                .map(|&global_idx| {
                    ...
                    let locked_until = if is_coinbase {
                        out.height + COINBASE_LOCK_WINDOW
                    } else {
                        out.height
                    };
                    ...
                    let Some(key) = monero_oxide::ed25519::CompressedPoint::from(out.key.to_bytes()).decompress() else {
                        return Ok(None);
                    };
                    let commitment = monero_oxide::ed25519::CompressedPoint::from(
