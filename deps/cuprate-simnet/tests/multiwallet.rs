...
//!
//! Proves that two `SimnetWallet` instances can send funds back and forth
//! through a single `SimnetNode` with correct balance accounting throughout.
...
async fn alice_sends_to_bob_and_bob_finds_output() {
    ...
    let mut bob = SimnetWallet::generate();
...
/// Alice.
///
/// `SimnetDecoyRpc::get_unlocked_outputs` applies the 60-block coinbase lock to
...
async fn bob_can_spend_what_alice_sent() {
    ...
    // SimnetDecoyRpc treats every RCT output as coinbase (60-block lock).
