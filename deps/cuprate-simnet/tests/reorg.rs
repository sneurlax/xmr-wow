...
use cuprate_simnet::{wallet::SimnetWallet, Simnet, SimnetNode};

// ── helpers ───────────────────────────────────────────────────────────────────

fn cd(node: &mut SimnetNode) -> u128 {
    node.cumulative_difficulty()
}

fn assert_wins(winner: &mut SimnetNode, loser: &mut SimnetNode, msg: &str) {
    let wcd = cd(winner);
    let lcd = cd(loser);
...
async fn selfish_private_chain_outpaces_honest_network() {
    let mut selfish = SimnetNode::start().await.unwrap();
    let mut honest = SimnetNode::start().await.unwrap();
...
async fn equal_forks_tie_on_cumulative_difficulty() {
    let mut a = SimnetNode::start().await.unwrap();
    let mut b = SimnetNode::start().await.unwrap();
...
async fn deep_reorg_requires_more_work_than_victim_chain() {
    ...
    let mut victim = SimnetNode::start().await.unwrap();
    victim.mine_blocks(depth).await.unwrap();
    let victim_cd = cd(&mut victim);

    let mut attacker = SimnetNode::start().await.unwrap();
...
async fn selfish_pool_dumps_withheld_chain_orphaning_honest_blocks() {
    let mut pool = SimnetNode::start().await.unwrap();
    let mut honest = SimnetNode::start().await.unwrap();
...
async fn partition_larger_mining_group_wins_on_heal() {
    ...
    let mut group_a = SimnetNode::start().await.unwrap();
    let mut group_b = SimnetNode::start().await.unwrap();
...
async fn network_convergence_after_sequential_mining() {
    let mut net = Simnet::new(3).await.unwrap();
...
async fn selfish_miner_collects_coinbase_on_winning_chain() {
    let selfish_wallet = SimnetWallet::generate();
...
async fn winning_losing_chains_hold_distinct_outputs() {
    ...
    let mut alice_node = SimnetNode::start().await.unwrap();
    let mut bob_node = SimnetNode::start().await.unwrap();
...
async fn majority_attacker_mines_coinbase_to_self() {
    let attacker_wallet = SimnetWallet::generate();
...
/// Mine a coinbase output then mine the 60 additional blocks required for
...
async fn chain_work_tracks_coinbase_maturity() {
    ...
    // Mine coinbase to wallet at height 1, then 60 more blocks (heights 2–61).
    node.mine_to(&wallet.spend_pub, &wallet.view_scalar, 1).await.unwrap();
    node.mine_blocks(60).await.unwrap();

    // cd == genesis(1) + 61 mined = 62.
    assert_eq!(node.cumulative_difficulty(), 62, "cd matches chain length");

    // Coinbase at height 1 unlocks at height 1 + 60 = 61.
    ...
    let premature = timelocked2.additional_timelock_satisfied_by(60, 0);
...
/// When `Simnet::mine_on_to` mines a block to a wallet on one node and
