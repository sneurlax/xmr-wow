use cuprate_simnet::{Simnet, SimnetConfig, SimnetError, SimnetNode, SimnetWallet};
use monero_wallet::{address::Network, interface::FeeRate};

#[cfg(feature = "merge-mining")]
use cuprate_simnet::TwoChainSimnet;
#[cfg(feature = "merge-mining")]
...
#[cfg(feature = "merge-mining")]
/// Re-parse a `monero_oxide::block::Block` (from the DB) as a `monero_oxide_mm::block::Block`
/// by going through the wire format. Both crates share the same serialization.
fn reparse_block(block: &monero_oxide::block::Block) -> monero_oxide_mm::block::Block {
    let bytes = block.serialize();
    monero_oxide_mm::block::Block::read(&mut bytes.as_slice())
...
async fn test_start_with_config() {
    let mut node = SimnetNode::start_with_config(SimnetConfig::mainnet()).await.unwrap();
    assert_eq!(node.height().await.unwrap(), 1);
}
...
async fn test_read_handle_cloneable() {
    let node = SimnetNode::start().await.unwrap();
...
async fn relay_compat_blob_yields_verifiable_proof() {
    ...
    use monero_oxide_mm::block::{Block, BlockHeader};
    use monero_oxide_mm::ed25519::CompressedPoint;
    use monero_oxide_mm::transaction::{Input, Output, Timelock, Transaction, TransactionPrefix};
    use std::io::Cursor;
    ...
    // Inject child hash using monero_oxide_mm::extra (same logic as relay after dedup).
    let enhanced = {
        let block = Block::read(&mut Cursor::new(&blob)).unwrap();
        let mut tx = block.miner_transaction().clone();
        tx.prefix_mut().extra =
            monero_oxide_mm::extra::set_merge_mining_hash(&tx.prefix().extra, child_hash);
...
async fn setup_alice_funded() -> (SimnetNode, SimnetWallet, SimnetWallet) {
    let mut node = SimnetNode::start().await.unwrap();
