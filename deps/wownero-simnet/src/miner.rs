...
use wownero_oxide::io::VarInt;

use crate::wallet::WOW_SPENDABLE_AGE;
...
fn into_consensus_block(block_blob: &[u8]) -> Result<monero_oxide::block::Block, SimnetError> {
    let mut block_blob = block_blob;
    monero_oxide::block::Block::read(&mut block_blob)
...
pub fn derive_coinbase_output(
    ...
    let shared_key: Scalar = wownero_oxide::ed25519::Scalar::hash(&output_derivation).into();
...
pub fn build_miner_tx_to_pubkeys(
    ...
    let prefix = TransactionPrefix {
        // WOW: 4-block coinbase lock (vs Monero's 60)
        additional_timelock: Timelock::Block(height + WOW_SPENDABLE_AGE),
...
/// stealth address that any `wownero_wallet::Scanner` holding the matching
...
pub fn produce_block(
    ...
    let block = into_consensus_block(&block_blob)?;
