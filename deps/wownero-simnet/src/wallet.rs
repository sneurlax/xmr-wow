//! Stateful Wownero wallet for simnet testing.
//!
//! Provides key generation, address derivation, block scanning, output
//! accumulation, and balance reporting against a live [`WowSimnetNode`].
//!
//! # WOW consensus differences from Monero
//! - Ring size: 22 (1 real + 21 decoys) vs Monero's 16
//! - Coinbase spendable age: 4 blocks vs Monero's 60
...
use crate::{error::SimnetError, node::{WowSimnetDecoyRpc, WowSimnetNode}};

/// Wownero ring size: 22 members (1 real + 21 decoys).
pub const WOW_RING_SIZE: u8 = 22;

/// Wownero coinbase spendable age: 4 blocks (vs Monero's 60).
pub const WOW_SPENDABLE_AGE: usize = 4;

/// A stateful Wownero wallet that can scan blocks produced by a [`WowSimnetNode`].
pub struct WowSimnetWallet {
    /// Private spend key.
...
impl WowSimnetWallet {
    ...
    pub fn view_pair(&self) -> Result<ViewPair, ViewPairError> {
        ViewPair::new(
            wownero_oxide::ed25519::Point::from(self.spend_pub),
            Zeroizing::new(wownero_oxide::ed25519::Scalar::from(*self.view_scalar)),
        )
    }
    ...
    pub async fn refresh(&mut self, node: &mut WowSimnetNode) -> Result<(), SimnetError> {
    ...
    /// i.e. what `WowSimnetNode::height()` returns cast to `usize`).
    ...
    /// Returns `CompressedPoint` = `wownero_oxide::ed25519::CompressedPoint`.
    pub fn key_image_for_output(&self, index: usize) -> Option<CompressedPoint> {
        ...
        let hp = wownero_oxide::ed25519::Point::biased_hash(output.key().compress().to_bytes());
    ...
    /// * `decoy_rpc` - a `WowSimnetDecoyRpc` obtained from `node.decoy_rpc()`
    ///
    /// WOW uses ring size 22 (1 real + 21 decoys). You must have at least
    /// `WOW_SPENDABLE_AGE + 21` blocks in the chain (4 for coinbase maturity
    /// + 21 for decoys) before this succeeds.
    ///
    ...
    pub async fn build_spend_tx(
        ...
        amount: u64,
        fee_rate: FeeRate,
        decoy_rpc: &WowSimnetDecoyRpc,
    ) -> Result<OxideTransaction, SimnetError> {
        let rct_type = RctType::WowneroClsagBulletproofPlus;
        let ring_len: u8 = WOW_RING_SIZE;

        let block_number = {
            use wownero_wallet::interface::ProvidesBlockchainMeta;
            decoy_rpc
                .latest_block_number()
                .await
        ...
        // Select decoys with WOW ring size.
        ...
        let oxide_spend_key =
            Zeroizing::new(wownero_oxide::ed25519::Scalar::from(*self.spend_scalar));
        let tx = signable
    ...
    ///
    /// Each output gets its own CLSAG ring (ring size 22). The chain must have
