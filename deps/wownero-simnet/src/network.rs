...
use crate::{error::SimnetError, miner::{mine_one_block_raw, mine_one_block_raw_to}, node::WowSimnetNode};

/// A multi-node WOW simulation network for testing block propagation.
pub struct WowSimnet {
    pub nodes: Vec<WowSimnetNode>,
}
...
impl WowSimnet {
    pub async fn new(n: usize) -> Result<Self, SimnetError> {
        ...
        for _ in 0..n {
            nodes.push(WowSimnetNode::start().await?);
        }
        Ok(Self { nodes })
    }
