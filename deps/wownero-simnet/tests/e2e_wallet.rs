...
use wownero_wallet::address::Network;
#[cfg(feature = "spend-tests")]
use wownero_wallet::interface::FeeRate;

// ── Key generation ────────────────────────────────────────────────────────────

#[tokio::test]
async fn test_wallet_generate() {
    let w = WowSimnetWallet::generate();
...
// ── Coinbase maturity / 4-block lock ─────────────────────────────────────────

#[tokio::test]
async fn test_coinbase_maturity_simulation() {
    // Canonical simnet use-case: mine coinbase to yourself, wait 4 blocks,
    ...
    // The coinbase at height 1 is locked until height 1 + 4 = 5.
    // Mine 4 more blocks (heights 2-5).
    node.mine_blocks(4).await.unwrap();
    assert_eq!(node.height().await.unwrap(), 6);

    let scannable = node.scannable_block_at(1).await.unwrap();
    let timelocked = scanner.scan(scannable).unwrap();

    // additional_timelock is Block(5). With chain tip at height 6, the lock
    // is satisfied at height 5.
    let matured = timelocked.additional_timelock_satisfied_by(5, 0);
    assert_eq!(matured.len(), 1, "coinbase output should be mature at height 5");
}
...
async fn test_coinbase_still_locked_before_maturity() {
    ...
    // Mine only 3 more blocks — one short of maturity.
    node.mine_blocks(3).await.unwrap();
    assert_eq!(node.height().await.unwrap(), 5);

    let scannable = node.scannable_block_at(1).await.unwrap();
    let timelocked = scanner.scan(scannable).unwrap();

    // At height 4 (one block before the lock expires at 5), it is still locked.
    let matured = timelocked.additional_timelock_satisfied_by(4, 0);
...
async fn test_wallet_does_not_accumulate_others_outputs() {
    let mut node = WowSimnetNode::start().await.unwrap();
    let mut wallet_a = WowSimnetWallet::generate();
    let mut wallet_b = WowSimnetWallet::generate();
