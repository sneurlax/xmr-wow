...
use xmr_wow_wallet::{verify_lock, CryptoNoteWallet, WowWallet, XmrWallet};
...
fn persist_and_reload(
    ...
    let password = b"phase16-proof-harness";
...
#[tokio::test]
async fn phase16_happy_path_uses_real_wallets_and_swap_state_on_simnet() {
    ...
    let bob_wow_artifact = WowWallet::new(testbed.wow_rpc_url())
    ...
    let alice_xmr_artifact = XmrWallet::new(testbed.xmr_rpc_url())
        .build_refund_artifact(
            &joint_spend_secret,
            &joint_view_scalar,
    ...
    let bob_completed_sig = bob_pre_sig.complete(&scalar_from_bytes(bob_secret)).unwrap();
    ...
    let alice_completed_sig = alice_pre_sig.complete(&scalar_from_bytes(alice_secret)).unwrap();
...
#[tokio::test]
async fn phase16_refund_artifact_survives_restart_and_enforces_height_boundary() {
    let testbed = SimnetTestbed::new().await.unwrap();
    let params = sample_params(
        testbed.xmr_height().await.unwrap(),
        testbed.wow_height().await.unwrap(),
    ...
    let bob_wow_locked = bob_joint.record_wow_lock(wow_lock_tx).unwrap();
    let artifact = WowWallet::new(testbed.wow_rpc_url())
        .build_refund_artifact(
            &joint_spend_secret,
            &joint_view_scalar,
            params.bob_refund_address.as_deref().unwrap(),
    ...
    let reloaded = persist_and_reload(
        &tempdir.path().join("phase16-refund.sqlite"),
        &bob_wow_locked,
        bob_secret,
    );
    ...
    let current_height = testbed.wow_height().await.unwrap();
    let blocks_needed = params.wow_refund_height.saturating_sub(current_height);
    if blocks_needed > 0 {
        testbed.mine_wow(blocks_needed).await.unwrap();
    }
