...
use crate::share::{Difficulty, EscrowOp, Hash, MergeMinedProof, SwapShare};
...
fn handle_submit_solution(state: &RpcState, id: Value, params: Option<Value>) -> Json<Value> {
    ...
    let merkle_path = params.get("path").and_then(Value::as_u64).unwrap_or(0) as u32;
...
fn handle_get_swap_status(state: &RpcState, id: Value, params: Option<Value>) -> Json<Value> {
    ...
    match idx.get(&swap_id) {
        ...
        Some(EscrowState::Refunded) => ok(id, serde_json::json!({ "state": "Refunded" })),
    }
}
...
fn handle_submit_escrow_op(state: &RpcState, id: Value, params: Option<Value>) -> Json<Value> {
    ...
    let share = SwapShare {
        parent: tip_id,
        uncles: Vec::new(),
        height: if state.chain.tip_id().is_some() {
            height + 1
        } else {
            0
        },
...
async fn handle_ws_socket(mut socket: WebSocket, state: RpcState, swap_id: [u8; 32]) {
    // Subscribe BEFORE reading history to avoid missing messages in the gap
    ...
    // Live stream with 30-second ping heartbeat
            ...
            result = rx.recv() => {
                match result {
                    ...
                    Err(broadcast::error::RecvError::Lagged(n)) => {
                        ...
                        // continue: next recv returns oldest retained message
                    }
        ...
        .filter_map(move |(i, result)| {
            ...
            async move {
                match result {
                    ...
                    Err(_) => {
                        // BroadcastStreamRecvError::Lagged: skip lost messages
                        None
                    }
                }
            }
...
fn handle_poll_coord_messages(state: &RpcState, id: Value, params: Option<Value>) -> Json<Value> {
    ...
    ok(
        id,
        serde_json::json!({ "messages": messages, "next_index": next_index }),
    )
}
...
fn handle_replay_coord_messages(state: &RpcState, id: Value, params: Option<Value>) -> Json<Value> {
    ...
    ok(
        id,
        serde_json::json!({ "messages": messages, "count": count }),
    )
}
...
pub fn check_loopback(
    ...
    if addr.ip().is_loopback() {
        ...
        Err(err(
            id,
            -32003,
            "submit_escrow_op is restricted to localhost",
        ))
    }
}
...
mod tests {
    ...
    async fn health_endpoint_returns_ok() {
        let router = merge_mining_router(make_chain());
        let resp = router
            .oneshot(
                Request::builder()
                    .uri("/health")
    ...
    fn submit_escrow_op_accepts_loopback() {
        ...
        let id = serde_json::json!(1);
        assert!(
            check_loopback(&loopback_v4, id).is_ok(),
        ...
        assert!(
            check_loopback(&loopback_v6, id2).is_ok(),
            "::1 must be accepted"
        );
    }
    ...
    async fn ws_isolation_by_swap_id() {
        ...
        // ws_b should NOT receive anything: timeout is expected
        let result = tokio::time::timeout(Duration::from_millis(200), ws_b.next()).await;
        assert!(
            result.is_err(),
            "swap_b subscriber should not receive swap_a messages"
        );
    }
    ...
    async fn sse_replays_then_live() {
        ...
        .await
        .unwrap_or_else(|_| {
            // Timeout is expected: we got the replay portion
            Ok(bytes::Bytes::new())
        })
        .unwrap();
        ...
        if body_bytes.is_empty() {
            ...
            assert!(
                body_text.contains("event: coord_message"),
                "missing event type"
            );
    ...
    async fn sse_event_format() {
        ...
        let body_text = if body_bytes.is_empty() {
            // SSE stream blocked: use TCP approach for format validation
    ...
    async fn sse_isolation_by_swap_id() {
        ...
        if body_bytes.is_empty() {
            // SSE stream blocked: use TCP approach
            let (addr, state2) = spawn_test_server().await;
            ...
            assert!(
                body_text.contains(&hex::encode(swap_b)),
                "should contain swap_b"
            );
            assert!(
                !body_text.contains(&hex::encode(swap_a)),
                "should NOT contain swap_a"
            );
        } else {
            let body_text = String::from_utf8(body_bytes.to_vec()).unwrap();
            assert!(
