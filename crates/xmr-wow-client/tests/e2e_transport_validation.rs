...
async fn sharechain_transport_full_message_flow() {
    ...
    alice_messenger.send(init_coord).await.expect("Alice sends Init");
    ...
    bob_messenger.send(response_coord).await.expect("Bob sends Response");

    let (raw_msgs, next_idx) = alice_client.poll_coord_messages(&swap_id, 1).await.unwrap();
    assert_eq!(raw_msgs.len(), 1, "Alice should see 1 new message (Response)");
    ...
    alice_messenger.send(adaptor_coord).await.expect("Alice sends AdaptorPreSig");

    let (raw_msgs, next_idx) = bob_client.poll_coord_messages(&swap_id, 2).await.unwrap();
    assert_eq!(raw_msgs.len(), 1, "Bob should see 1 new message (AdaptorPreSig)");
    ...
    bob_messenger.send(claim_coord).await.expect("Bob sends ClaimProof");

    let (raw_msgs, next_idx) = alice_client.poll_coord_messages(&swap_id, 3).await.unwrap();
    assert_eq!(raw_msgs.len(), 1, "Alice should see 1 new message (ClaimProof)");
    ...
    assert_eq!(all_msgs.len(), 4, "full replay from index 0 must return all 4 messages");
    ...
    for (i, raw) in all_msgs.iter().enumerate() {
        ...
        assert_eq!(variant, expected_variants[i], "replay message {} must be {}", i, expected_variants[i]);
    }
}
...
fn oob_transport_full_message_flow_no_sharechain() {
    use xmr_wow_client::{encode_message, decode_message};
    ...
    for (i, msg) in msgs.iter().enumerate() {
        let coord = wrap_protocol_message(swap_id, msg)
            .expect("wrap_protocol_message must succeed");

        let encoded_inner = encode_message(msg);
        assert!(encoded_inner.starts_with("xmrwow1:"), "encoded message must have xmrwow1: prefix");

        let json_bytes = serde_json::to_vec(&coord).expect("CoordMessage must serialize to JSON");
        assert!(!json_bytes.is_empty(), "serialized CoordMessage must not be empty");
        ...
        assert_eq!(
            ...
            "swap_id must survive OOB round-trip for message {}", i
        );

        let recovered = unwrap_protocol_message(&deserialized)
        ...
        assert_eq!(
            ...
            "OOB recovered variant must match original for message index {}", i
        );
        ...
        assert_eq!(
            ...
            "encode/decode round-trip must preserve variant for message index {}", i
        );
    }

    assert_eq!(msgs.len(), 4, "all four ProtocolMessage variants must be covered");
}
