...
mod tests {
    ...
    #[test]
    #[allow(non_snake_case)] // Mathematical convention: uppercase K = EC point, lowercase k = scalar
    ...
    /// Regression: reject addresses with torsion-component public keys.
    #[test]
    fn test_decode_address_rejects_torsion_point() {
        use curve25519_dalek::edwards::CompressedEdwardsY;

        // 2-torsion point (0, -1) on Ed25519: y = p - 1 in little-endian.
        // p = 2^255 - 19; p - 1 = 2^255 - 20.
        ...
        let two_torsion_point = CompressedEdwardsY::from_slice(&two_torsion_bytes)
            .expect("two-torsion bytes must be valid compressed form")
            .decompress()
            .expect("two-torsion point must decompress");
        assert!(
            !two_torsion_point.is_torsion_free(),
            "2-torsion point (0, -1) must NOT be torsion-free"
        );
        ...
        assert!(
            result.is_err(),
            "decode_address must reject an address with a torsion spend key"
        );
        assert_eq!(
            result.unwrap_err(),
            crate::error::CryptoError::NonPrimeOrderPoint,
            "error must be NonPrimeOrderPoint"
        );
    }
}
